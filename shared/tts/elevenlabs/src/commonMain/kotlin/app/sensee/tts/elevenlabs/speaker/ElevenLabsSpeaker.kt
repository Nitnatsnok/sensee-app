...
import app.sensee.tts.elevenlabs.synthesis.ElevenLabsSynthesizer
...
/**
 * Plays audio from a [SpeechSynthesizer] (an [ElevenLabsSynthesizer], optionally
 * wrapped in a caching decorator) through a platform [AudioPlayer].
 *
 * One synthesizer + one factory; the speaker creates a fresh player per request
 * so that overlapping `speak` calls cancel cleanly.
 */
public class ElevenLabsSpeaker(
    private val synthesizer: SpeechSynthesizer,
    private val playerFactory: AudioPlayerFactory,
    private val scope: CoroutineScope,
    logger: AppLogger,
) : Speaker {
    private val log = logger.tag("ElevenLabsSpeaker")
    private var currentJob: Job? = null
    private var currentPlayer: AudioPlayer? = null

    override fun speak(request: SpeechRequest): SpeechHandle {
        stop()
        val handle = ElevenLabsSpeechHandle()
        val job =
            scope.launch {
    ...
    private fun createAttachedPlayer(handle: ElevenLabsSpeechHandle): AudioPlayer =
...
private class ElevenLabsSpeechHandle : SpeechHandle {
