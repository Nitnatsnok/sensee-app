...
/**
 * Per-example aggregate finding. An adapter that runs spell/grammar/style
 * checks against an example may emit several [ExampleIssue]s plus an optional
 * full rewrite — the rewrite is independent of the per-issue resolution so
 * the orchestrator can apply only the rewrite, only the issues, or neither.
 */
@Serializable
public data class ExampleFinding(
    val senseHintId: String?,
