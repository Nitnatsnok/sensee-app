 ...
 * only what disambiguates a sense (surface form, part-of-speech badge under
 * the word, translation, explanation, one example); the rest of the grammar
...
internal fun SenseSelectionCard(
    ...
    SenseeSurface(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(spacing.small)) {
            SenseCandidateTitleRow(candidate = candidate, selected = selected)
            ...
            if (candidate.hasSecondaryDetail()) {
                SenseeButton(
                    onClick = { expanded = !expanded },
                    colors = SenseeButtonColors.text(),
                ) {
                    Text(
                        textProvider.text(
                            if (expanded) {
            ...
            if (expanded) {
                SenseSecondaryDetail(candidate, labels, textProvider)
            }
...
@Composable
private fun ChipFlow(chips: ImmutableList<String>) {
    val spacing = SenseeTheme.spacing
    FlowRow(
        horizontalArrangement = Arrangement.spacedBy(spacing.extraSmall),
        ...
        chips.forEach { SenseeBadge(text = it) }
    }
}

private fun MeaningCandidate.hasSecondaryDetail(): Boolean =
