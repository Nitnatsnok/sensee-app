...
Функция: Профиль и пользовательские настройки
  ...
  - границы AI/TTS-интеграций `shared/ai/*` / `shared/tts/*` (ADR-005): список
    совместимых провайдеров и выбор модели (проверка через API провайдера,
    короткий curated-список для enrichment, офлайн-fallback)
  ...
  Сценарий: Двухпанельная раскладка Profile на широких экранах
    ...
    Когда пользователь открывает раздел `Profile`
    Тогда показано меню и экран категории рядом во второй панели
    И если целевая категория не задана, во второй панели открыта категория `App`
    И вторая панель на широком экране остаётся открытой: системная «назад» не закрывает категорию
    И выбор другого пункта меню заменяет содержимое второй панели
  ...
  Сценарий: Проверка ключа AI по кнопке открывает выбор модели
    ...
    И при валидном ключе показывается короткий список совместимых моделей для enrichment
