...
- `pre-commit` blocks generated output, local files, secrets, merge markers, trailing whitespace, missing final newlines, and oversized staged files. Two sets of generated artifacts are intentionally tracked and exempt from the generated-output block. `graphify-out/graph.json` and `graphify-out/GRAPH_REPORT.md` are skipped entirely (large and not newline-terminated); `graphify-rebuild` regenerates them from staged changed paths and stages them in the same `pre-commit` run, falling back to a temporary staged-index checkout only when the same path has both staged and unstaged edits. `kotlin-js-store/yarn.lock` and `kotlin-js-store/wasm/yarn.lock` are the committed yarn lockfiles (regenerated by `kotlinUpgradeYarnLock`, validated by `:kotlinStoreYarnLock` in CI); they still pass the text guards, and the rest of `kotlin-js-store/` stays blocked.
